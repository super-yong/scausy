...
function sentEmail($to,$subject,$body){
  try {
    ...
    $mail->IsSMTP();
    $mail->AddReplyTo("15016291381@163.com","思源工作室");//回复地址
    $mail->FromName = "思源工作室秘书部";
    $mail->AddAddress($to);
...
function phpExcel($data){
    ...
    $PHPExcel->getProperties()->setCreator("lcy")
    ->setLastModifiedBy("lcy")
    ->setTitle("scau_sxr_sy")
    ->setSubject("")
    ->setDescription("")
    ->setKeywords("")
    ->setCategory("");
    ...
    foreach($data as $k=>$v){
    $PHPExcel->setActiveSheetIndex(0)
      ->setCellValue('A'.$i, $v['college'])
      ->setCellValue('B'.$i, $v['id'])
      ->setCellValue('C'.$i, $v['name'])
      ->setCellValue('D'.$i, $v['major'])
      ->setCellValue('E'.$i, $v['class'])
      ->setCellValue('F'.$i, $v['email'])
      ->setCellValue('G'.$i, $v['phone']);
    $i++;
    }
    $PHPExcel->getActiveSheet()->setTitle('思源工作室');//设置sheet标签的名称
    $PHPExcel->setActiveSheetIndex(0);
    ob_end_clean();  //清空缓存 
    ...
    header('Content-Disposition:attachment;filename=思源工作室.xls');//设置文件的名称
...
function truncate($string,$length=0,$ellipsis='…',$start=0){
...
function week($beginTime,$endTime,$beginWeek){
    $startWeek = date('W',$beginTime);
    $endWeek = date('W',$endTime);
    if ($startWeek>$endWeek) {
        $endWeek += 53; 
    }
    return $endWeek - $startWeek + $beginWeek;
  }
